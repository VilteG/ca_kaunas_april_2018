...
$('#sortNameBtn').click(function(){ // sort by name, isrikiuoja pagal abecele java script sort array
	produktai = sortByName(produktai);
	printProduktai(produktai);
});

$('#sortPriceBtn').click(function(){ // sort by price, isrikiuoja pagal kaina nuo maziausios java script sort array
	produktai = sortByPrice(produktai);	
	printProduktai(produktai); // negali isimti, pasiaiskink

});

$('#filterCityBtn').click(function() {
	produktai = produktai.filter(function(prod){
	 // negali isimti, pasiaiskink
	return prod.miestas == "Vilnius";
});
	printProduktai(produktai);
});
...
function sortByPrice (array){
	array.sort(
		function(a, b) {
			return a.kaina > b.kaina // rusiavimas pagal kaina
		}
	);
return array;
}